...
Upon completion of data tidying and transformation, the next phase of analysis is to visualize the data. The initial visualization was to map the data using the [ggmap() package](https://cran.r-project.org/web/packages/ggmap/readme/README.html). The banding data point distribution accurately reflects the known species distribution of both species of albatross. The only two main Hawaiin islands where albatross nest at Kauai and Oahu. Disappointingly, I was unable to obtain a basemap that displayed the entire Hawaii archipelago. Using the [geom_label_repel() function](https://www.rdocumentation.org/packages/ggrepel/versions/0.9.1/topics/geom_label_repel), I was able to place labels adjacent to all locations within Papahanaumokuakea. 
